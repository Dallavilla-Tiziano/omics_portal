...
from .tables import PatientTable, SampleTable, AnalysisTable
...
def get_filter_counts(request):
    ...
    return {
        ...
        "samples": Sample.objects.filter(patient__in=filtered_patients).distinct().count(),
        "analyses": Analysis.objects.filter(samples__patient__in=filtered_patients).distinct().count(),
    }
...
class PatientListView(LoginRequiredMixin, SingleTableMixin, FilterView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["demographic_filter"] = DemographicFilter(self.request.GET, queryset=Patient.objects.all())
        context["sample_filter"] = SampleFilter(self.request.GET, queryset=Sample.objects.all())
        context["analysis_filter"] = AnalysisFilter(self.request.GET, queryset=Analysis.objects.all())
