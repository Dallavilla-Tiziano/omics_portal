...
#########################################################################################
### \\\\\ CLINICAL STATUS BLOCK: 
### \\\ It should contain info related to:
### \\\ - Clinical evaluation: associated to a visit with a doctor 
### \\\ - Comorbidities: collection of pathologies associated (in theory, not cardiovascular pathologies)
### \\\ - Risk factors: info related to lifestyle
...
class Events(Clinical_Status):

	class Resuscitatedcardiacarrest(models.TextChoices):
		Y = "Yes", "Yes"
		N = "No", "No"	
	resuscitated_cardiac_arrest = models.CharField(
		max_length=3,
		choices=Resuscitatedcardiacarrest
	)
	
	class Arrhythmias(models.TextChoices):
		Y = "Yes", "Yes"
		N = "No", "No"
	arrhythmias = models.CharField(
		max_length=3,
		choices=Arrhythmias
	)
	# !!! Only if "Yes" is selected, these should be compiled !!!
	class TypeArrhythmias(models.TextChoices):
		svt = "SVT", "SVT"
		vt = "VT/VF", "VT/VF"
	type_of_arrhythmias = models.CharField(
		max_length=6,
		choices=TypeArrhythmias
	)

	class CardiacTransplantation(models.TextChoices):
		Y = "Yes", "Yes"
		N = "No", "No"	
	cardiac_transplantation = models.CharField(
		max_length=3,
		choices=CardiacTransplantation
	)

	class StrokeTIA(models.TextChoices):
		Y = "Yes", "Yes"
		N = "No", "No"	
	Stroke_or_TIA = models.CharField(
		max_length=3,
		choices=StrokeTIA
	)

	class Death(models.TextChoices):
		Y = "Yes", "Yes"
		N = "No", "No"	
	...
	# !!! Do we want to create an elencation of options? !!!


    # !!! It covers the necessity of "Other" option ... do we want to keep it? !!!
	class OtherFU(models.TextChoices):
		Y = "Yes", "Yes"
		N = "No", "No"	
	other_FU = models.CharField(
		max_length=3,
		choices=OtherFU,
		default=''
	)
	type_of_other_FU = models.CharField(max_length=100, blank=True, default='')


    # !!! It seems like subcathegory of events... is it ok or do we want to create a voice like "way of event communication?"
	# with possible options like "alert", "phone call", ... !!!
	class RMalert(models.TextChoices):
		Y = "Yes", "Yes"
		N = "No", "No"	
	rm_alert = models.CharField(
		max_length=3,
		choices=RMalert,
		default=''
	)
	# !!! Only if "Yes" is selected, these should be compiled !!!
	class RMalertType(models.TextChoices):
		ar = "Arrhythmia", "Arrhythmia"
		lb = "Low battery", "Low battery"
		lcs = "Low CRT stimulation", "Low CRT stimulation"
		hd = "Heart failure", "Heart failure"
		int = "Interference", "Interference"
		n = "Noise", "Noise"
		p = "Pause", "Pause"
		burd = "Burden", "Burden"
		unsen = "Undersensing", "Undersensing"
		br = "Bradycardia", "Bradycardia"
		dbp = "Decreasing biventricular pacing", "Decreasing biventricular pacing"
		drvs = "Decreasing RV sensing", "Decreasing RV sensing"		
	type_of_rm_alert = models.CharField(
		max_length=32,
		choices=RMalertType,
		default=''
	)
	# !!! In Canva there are way more options... which we do we need? !!!

	doc_indications = models.CharField(max_length=200, blank=True, default='')
	# !!! Indications from Prof or Doc. Locati are put in input as CharField.. 
	# it's kind of a note, but we should design it in a more structured way !!!
	# !!! That's the same for "Event description"!!!

	# !!! In Canva there's also the link to last visit file... do we want to implement it and how? !!!
